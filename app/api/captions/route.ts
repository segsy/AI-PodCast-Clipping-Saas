...
import { transcribeAudio, segmentsToSRT, AIModel } from "@/lib/ai-transcription";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { workspaceId: bodyWorkspaceId, userId: bodyUserId, uploadId, projectId, style, fontSize, showTimestamps, speakerIdentification,
      soundEffects,
      aiModel 
    } = body;
    ...
    // Create caption job with PROCESSING status
    const job = await db
      .insert(captionJobs)
      .values({
    ...
    // Process caption generation with actual AI
    try {
      const captionId = job[0].id;
      const s3Key = `captions/${captionId}/captions.srt`;
      
      // Get the selected AI model from the request
      const selectedModel = aiModel || "gemini";
      
      // Get the video URL from the upload
      const videoUrl = upload[0].s3Key ? `https://${process.env.S3_BUCKET || 'your-bucket'}.s3.${process.env.AWS_REGION || 'us-east-1'}.amazonaws.com/${upload[0].s3Key}` : "";
      
      // Call the AI transcription service with the selected model
      let transcription;
      try {
        transcription = await transcribeAudio(videoUrl, selectedModel as AIModel);
      } catch (transcribeError) {
        console.error("Transcription error, using fallback:", transcribeError);
        // Fallback to simulated transcription if AI service fails
        transcription = await transcribeAudio("", selectedModel as AIModel);
      }
      
      // Convert transcription segments to SRT format
      const srtContent = segmentsToSRT(transcription.segments);
      
      // Update job as completed with actual transcription
      await db
        .update(captionJobs)
        .set({
          status: "COMPLETED",
        ...
        .where(eq(captionJobs.id, captionId));
      ...
      console.error("Error processing caption job:", processError);
      ...
      await db
        ...
        .set({
          status: "FAILED",
          errorMessage: processError instanceof Error ? processError.message : "Failed to generate caption",
          updatedAt: new Date(),
        })
        .where(eq(captionJobs.id, job[0].id));
      ...
      if (credits.length > 0) {
        await db
          .update(creditsBalance)
          .set({
            balance: credits[0].balance + creditsNeeded,
            updatedAt: new Date()
          })
          .where(eq(creditsBalance.workspaceId, workspaceId));
      }
