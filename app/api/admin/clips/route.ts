...
import { eq, desc, like, or, and, count, asc, inArray } from "drizzle-orm";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const rawJobIds = allClips.map(c => c.jobId);
    const jobIds = rawJobIds.filter((id): id is string => id !== null);
    // Additional safety: ensure we only pass string array to eq()
    const validJobIds: string[] = jobIds;
    ...
    if (validJobIds.length > 0) {
      jobDetails = await db
        ...
        .from(clipJobs)
        .where(inArray(clipJobs.id, validJobIds));
    }
