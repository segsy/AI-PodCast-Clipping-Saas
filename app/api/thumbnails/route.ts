...
import { generateThumbnails, AIModel } from "@/lib/ai-thumbnail";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { workspaceId: bodyWorkspaceId, userId: bodyUserId, uploadId, projectId, style, aspectRatio, titleText, addTitle, addEpisodeNumber, addGlowEffect, aiModel, variants } = body;
    ...
    // Create thumbnail job with PROCESSING status
    const job = await db
    ...
    // Process thumbnail generation with actual AI
    try {
      const thumbnailId = job[0].id;
      
      // Get the selected AI model
      const selectedModel = (aiModel || "gemini") as AIModel;
      
      // Call the AI thumbnail generation service with the selected model
      let thumbnailResult;
      try {
        thumbnailResult = await generateThumbnails(
          titleText || "Video Thumbnail",
          style || "vibrant",
          aspectRatio || "16:9",
          numVariants,
          selectedModel
        ...
        console.error("Thumbnail generation error, using fallback:", genError);
        // Fallback to simulated generation if AI service fails
        thumbnailResult = await generateThumbnails(
          titleText || "Video Thumbnail",
          style || "vibrant",
          aspectRatio || "16:9",
      ...
      // Extract the S3 keys from the generated variants
      const variants = thumbnailResult.variants.map(v => v.s3Key);
      
      // Update job as completed with actual AI-generated thumbnails
      await db
        ...
        .where(eq(thumbnailJobs.id, thumbnailId));
      ...
      console.error("Error processing thumbnail job:", processError);
      ...
      await db
        ...
        .set({
          status: "FAILED",
          errorMessage: processError instanceof Error ? processError.message : "Failed to generate thumbnails",
          updatedAt: new Date(),
        })
        .where(eq(thumbnailJobs.id, job[0].id));
