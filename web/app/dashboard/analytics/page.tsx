...
export default function AnalyticsPage() {
  ...
  const [hasData, setHasData] = useState(false); // Track if we have actual data
  const [exporting, setExporting] = useState(false);
  const [toast, setToast] = useState<{ message: string; type: 'success' | 'error' } | null>(null);

  useEffect(() => {
    console.log('[DEBUG] Analytics: Starting to fetch data');
    console.log('[DEBUG] Analytics: timeRange =', timeRange);
    
    const fetchAnalytics = async () => {
      setLoading(true);
      try {
        const workspaceId = localStorage.getItem("workspaceId") || "demo-workspace";
        console.log('[DEBUG] Analytics: Fetching from API with workspaceId =', workspaceId);
        
        const response = await fetch(`/api/analytics?workspaceId=${workspaceId}&period=${timeRange}`);
        console.log('[DEBUG] Analytics: Response status =', response.status);
        
        if (response.ok) {
          const data = await response.json();
          console.log('[DEBUG] Analytics: API returned data:', data);
          setAnalyticsData(data);
          // Check if we have actual data in the summary
          const hasActualData = data?.summary && (
            data.summary.totalViews > 0 ||
            data.summary.totalLikes > 0 ||
            data.summary.totalShares > 0 ||
            data.summary.totalComments > 0 ||
            data.summary.totalNewFollowers > 0
          );
          setHasData(hasActualData);
          console.log('[DEBUG] Analytics: Has actual data =', hasActualData);
        } else {
          console.log('[DEBUG] Analytics: API returned error:', response.status);
          setHasData(false); // No data on API error
          // Try to get error message
          try {
            const errorData = await response.json();
            console.log('[DEBUG] Analytics: Error details:', errorData);
          } catch(e) {
            // ignore
          }
        }
      } catch (error) {
        console.log('[DEBUG] Analytics: Using mock data - API error:', error);
        setHasData(false); // No data on exception
      } finally {
        setLoading(false);
        console.log('[DEBUG] Analytics: Loading complete');
      }
    };
  ...
  const handleExport = async (format: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/analytics?workspaceId=${workspaceId}&period=${timeRange}&export=true`);
      ...
      if (response.ok) {
        ...
        if (format === 'csv') {
          ...
          const rows = [
            ['Total Views', exportData.summary.totalViews],
            ['Total Likes', exportData.summary.totalLikes],
            ['Total Shares', exportData.summary.totalShares],
            ['Total Comments', exportData.summary.totalComments],
            ['New Followers', exportData.summary.totalNewFollowers],
            ['Avg Watch Time', exportData.summary.avgWatchTime],
            ['Total Videos', exportData.summary.totalVideos],
            ['Engagement Rate', exportData.summary.engagementRate + '%'],
          ];
      ...
      a.download = `analytics-report-${timeRange}-${new Date().toISOString().split('T')[0]}.${ext}`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      
      setToast({ message: `Analytics report for ${timeRange} exported successfully!`, type: 'success' });
      
    } catch (error) {
      console.error("Export failed:", error);
      setToast({ message: "Failed to export analytics report", type: 'error' });
    } finally {
      setExporting(false);
    }
  };

  // Use empty data when hasData is false (no actual data from API)
  const chartData = hasData && analyticsData && analyticsData.dailyData && analyticsData.dailyData.length > 0 
    ...
    : // Return empty chart data (all zeros) when no real data
  ...
  // Use 0 when no actual data, otherwise use API data or mock as fallback
  const totalViews = hasData && analyticsData?.summary?.totalViews 
    ? analyticsData.summary.totalViews 
    : 0;
  const totalLikes = hasData && analyticsData?.summary?.totalLikes 
    ? analyticsData.summary.totalLikes 
    : 0;
  const totalShares = hasData && analyticsData?.summary?.totalShares 
    ? analyticsData.summary.totalShares 
    : 0;
  const totalNewFollowers = hasData && analyticsData?.summary?.totalNewFollowers 
    ? analyticsData.summary.totalNewFollowers 
    : 0;
  const avgWatchTime = hasData && analyticsData?.summary?.avgWatchTime 
    ? `${Math.floor(analyticsData.summary.avgWatchTime / 60)}:${String(Math.floor(analyticsData.summary.avgWatchTime % 60)).padStart(2, '0')}`
    : "0:00";

  // Platform distribution data - use API data if available, otherwise show 0
  const platformStats = hasData && analyticsData?.platformPerformance && analyticsData.platformPerformance.length > 0
    ? analyticsData.platformPerformance.map((p: any) => ({
        name: p.platform === 'YOUTUBE' ? 'YouTube Shorts' : p.platform === 'TIKTOK' ? 'TikTok' : 'Instagram Reels',
        views: p.views > 0 ? `${(p.views / 1000).toFixed(1)}K` : '0',
        percentage: p.views > 0 ? Math.round((p.views / (analyticsData.summary.totalViews || 1)) * 100) : 0,
        color: p.platform === 'YOUTUBE' ? '#FF0000' : p.platform === 'TIKTOK' ? '#000000' : '#E1306C',
      }))
    : [
        { name: "YouTube Shorts", views: "0", percentage: 0, color: "#FF0000" },
        { name: "TikTok", views: "0", percentage: 0, color: "#000000" },
        { name: "Instagram Reels", views: "0", percentage: 0, color: "#E1306C" },
      ];

  // Top clips data - use API data if available, otherwise show empty
  const topClips = hasData && analyticsData?.topClips && analyticsData.topClips.length > 0
    ? analyticsData.topClips.slice(0, 5).map((c: any) => ({
        id: c.id,
        title: c.title || 'Untitled',
        views: c.score > 0 ? `${(c.score / 1000).toFixed(1)}K` : '0',
        growth: "+0%",
        platform: "YouTube",
      }))
    : [];
            ...
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: "16px" }}>
                ...
                <div style={{ backgroundColor: "var(--surface)", borderRadius: "12px", border: "1px solid var(--border)", padding: "20px" }}>
                  ...
                  <p style={{ fontSize: "28px", fontWeight: "bold", color: "white" }}>{totalNewFollowers > 0 ? `+${(totalNewFollowers / 1000).toFixed(1)}K` : '0'}</p>
                  <div style={{ display: "flex", alignItems: "center", gap: "4px", marginTop: "4px" }}>
                    <ArrowUpRight size={14} style={{ color: hasData ? "var(--success)" : "var(--text-muted)" }} />
                    <span style={{ fontSize: "12px", color: hasData ? "var(--success)" : "var(--text-muted)", fontWeight: 500 }}>{hasData ? "+45%" : "No data"}</span>
                    <span style={{ fontSize: "12px", color: "var(--text-muted)" }}>{hasData ? "vs last week" : ""}</span>
                  </div>
                </div>
              </div>
                ...
                <div style={{ backgroundColor: "var(--surface)", borderRadius: "12px", border: "1px solid var(--border)", padding: "24px" }}>
                  ...
                  <div style={{ marginTop: "24px", paddingTop: "16px", borderTop: "1px solid var(--border)" }}>
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      ...
                      <span style={{ fontSize: "16px", fontWeight: 600, color: "white" }}>{hasData ? `${(totalViews / 1000).toFixed(1)}K` : '0'}</span>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div style={{ display: "flex", flexDirection: "column" }}>
                  {topClips.length > 0 ? (
                    ...
                    <div style={{ padding: "40px 20px", textAlign: "center" }}>
                      <p style={{ color: "var(--text-muted)", fontSize: "14px" }}>No clips found</p>
                      <p style={{ color: "var(--text-muted)", fontSize: "12px", marginTop: "8px" }}>Upload and process videos to see top performing clips</p>
                    </div>
                  )}
                </div>
              </div>
