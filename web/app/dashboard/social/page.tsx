...
import { getCurrentSubscription, PLANS } from "@/lib/billing";
...
const getPlatformInfo = (platformKey: string) => {
  return allPlatforms.find(p => p.key === platformKey) || { name: platformKey, key: platformKey, icon: "ðŸ“±", color: "#666666" };
};
...
export default function SocialAccountsPage() {
  ...
  useEffect(() => {
    ...
    if (success === "connected") {
      setToast({ message: "Social account connected successfully!", type: 'success' });
      // Clean up URL
      ...
      const errorMessages: Record<string, string> = {
        ...
        "oauth_not_configured": "OAuth not configured for this platform",
        "token_exchange_failed": "Failed to exchange authentication code",
        "callback_failed": "Authentication failed",
        "unsupported_platform": "Platform not supported"
      };
      setToast({ message: errorMessages[error] || "Failed to connect account", type: 'error' });
      window.history.replaceState({}, '', '/dashboard/social');
    }
  }, []);
  ...
  useEffect(() => {
    const checkSubscription = async () => {
      try {
        ...
        // Continue with null subscription - users without subscription can't connect accounts
      } finally {
        setLoading(false);
      }
      
      // Fetch connected accounts from database - do this separately from subscription
      try {
  ...
  const isProUser = subscription && (
    subscription.planId === PLANS.pro.priceIdMonthly ||
    subscription.planId === PLANS.pro.priceIdAnnual ||
    subscription.planId === PLANS.business.priceIdMonthly ||
    subscription.planId === PLANS.business.priceIdAnnual
  );

  // Allow all authenticated users to connect social accounts
  const canConnectAccounts = true;
  ...
  const handleAddAccount = async () => {
    ...
    try {
      ...
      const oauthUrl = `${baseUrl}/api/social-accounts/oauth?platform=${platformKey}&workspaceId=${workspaceId}`;
      ...
      setToast({ message: "Failed to connect account", type: 'error' });
    } finally {
      setConnecting(false);
    }
  };
  ...
  const handleDisconnectAccount = async (accountId: string) => {
    try {
      ...
      setToast({ message: "Social account disconnected successfully!", type: 'success' });
    } catch (error) {
        ...
        <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
          ...
          <button 
            ...
            style={{
              ...
              gap: "8px",
              padding: "10px 20px",
              backgroundColor: canConnectAccounts ? "var(--primary)" : "var(--surface)",
              border: canConnectAccounts ? "none" : "1px solid var(--border)",
              borderRadius: "8px",
              color: canConnectAccounts ? "white" : "var(--text-muted)",
              fontWeight: 500,
              cursor: "pointer",
              opacity: canConnectAccounts ? 1 : 0.6
            }}
          >
            {canConnectAccounts ? <Plus size={18} /> : <Lock size={18} />}
            {canConnectAccounts ? "Add Account" : "Upgrade to Pro"}
          </button>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ fontSize: "12px", color: "var(--text-secondary)" }}>
            Link your social media accounts to easily share your clips
          </p>
        </div>
      </div>
                ...
                <div key={platform.name} style={{
                  ...
                  <button 
                    onClick={() => {
                      ...
                      const baseUrl = window.location.origin;
                      window.location.href = `${baseUrl}/api/social-accounts/oauth?platform=${platformKey}&workspaceId=${workspaceId}`;
                    }}
                    style={{
                      padding: "6px 12px",
          ...
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "24px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
                ...
                <div>
                  <h2 style={{ fontSize: "24px", fontWeight: "bold", color: "white", marginBottom: "4px" }}>Connect Social Accounts</h2>
                  <p style={{ fontSize: "14px", color: "var(--text-secondary)" }}>Link your social media accounts</p>
                </div>
              </div>
              <button 
            ...
            <p style={{ color: "var(--text-secondary)", fontSize: "14px", marginBottom: "24px" }}>
              Connect your social accounts to easily share your clips:
            </p>
