...
export async function GET(request: NextRequest) {
  try {
    ...
    if (status && status !== "all") {
      ...
      conditions.push(eq(clips.status, statusValue as typeof clips.status.enumValues[number]));
    }
    ...
    const validProjectIds = projectIds.filter((id): id is string => id !== null);
    if (validProjectIds.length > 0) {
      projectDetails = await db
        ...
        .from(projects)
        .where(or(...validProjectIds.map(id => eq(projects.id, id))));
    }
    
    // Get workspaces
    let workspaceDetails: any[] = [];
    const validWorkspaceIds = workspaceIds.filter((id): id is string => id !== null);
    if (validWorkspaceIds.length > 0) {
      workspaceDetails = await db
        ...
        .from(workspaces)
        .where(or(...validWorkspaceIds.map(id => eq(workspaces.id, id))));
    }
    ...
    if (validJobIds.length > 0) {
      jobDetails = await db
        ...
        .from(clipJobs)
        .where(or(...validJobIds.map(id => eq(clipJobs.id, id))));
    }
