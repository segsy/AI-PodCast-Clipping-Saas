...
// Debug: Log environment variable presence at startup
...
console.log("[AUTH DEBUG] AUTH_GITHUB_ID:", process.env.AUTH_GITHUB_ID ? "SET" : "MISSING");
console.log("[AUTH DEBUG] AUTH_GITHUB_SECRET:", process.env.AUTH_GITHUB_SECRET ? "SET" : "MISSING");
console.log("[AUTH DEBUG] NEXTAUTH_SECRET:", process.env.NEXTAUTH_SECRET ? "SET" : "MISSING");
console.log("[AUTH DEBUG] NEXTAUTH_URL:", process.env.NEXTAUTH_URL);
console.log("[AUTH DEBUG] AUTH_TRUST_HOST:", process.env.AUTH_TRUST_HOST);*/
    ...
    async signIn({ user, account, profile }) {
      ...
      try {
        ...
        if (userEmail) {
          ...
          const result = await sql`SELECT id, email, name, avatar_url 
             FROM users 
          ...
          if (!existingUser) {
            ...
            await sql`INSERT INTO users (id, email, name) VALUES (${newUserId}, ${userEmail}, ${user.name || null})`;
            ...
            await sql`UPDATE users SET name = ${user.name || null} WHERE id = ${existingUser.id as string}`;
